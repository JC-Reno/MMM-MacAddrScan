...
git clone https://gitlab.com/JC-Reno/MMM-MacAddrScan.git
```

Install the dependencies (`sudo`, `ping`) in the MMM-MacAddrScan module folder:
```console
cd ~/MagicMirror/modules/MMM-MacAddrScan
npm install
```
...
```console
cd ~/MagicMirror/modules/MMM-MacAddrScan/scripts
chmod +x arps2mm.sh
./arps2mm.sh
```
...
The default styling applied to the MMM-MacAddrScan module is as follows:

```css
.MMM-MacAddrScan .deviceCell {
  text-align: left;
}

.MMM-MacAddrScan .deviceCell i {
  padding-left: 0;
  padding-right: 10px;
}

.MMM-MacAddrScan .dateCell {
  padding-left: 30px;
  text-align: right;
}
```
...
You can dynamically control some MMM-MacAddrScan configuration settings
...
used with MMM-MacAddrScan are already created by the module. You only need
to follow the [4th Party Modules Wiki Installation instructions](http://wiki.bugsounet.fr/en/MMM-TelegramBot/Installation).

### Telegram usage
Once installed and configured, you can control your MMM-MacAddrScan display
...
MMM-MacAddrScan module folder and type the following command:
...
<p float="left">
Network/System Info Screen and MMM-MacAddrScan Module<br/>
...
- [MagicMirror/config/config.js](https://gitlab.com/JC-Reno/MirrorCommand/-/blob/master/config/config-networkcols.js)
- [MagicMirror/css/custom-network-cols.css](https://gitlab.com/JC-Reno/MirrorCommand/-/blob/master/css/custom-network-cols.css)
...
- [MagicMirror/modules/MMM-MacAddrScan](https://gitlab.com/JC-Reno/MMM-MacAddrScan)

## Support

Issues with MMM-MacAddrScan can be reported and tracked in the Issues section
of the repository at https://gitlab.com/JC-Reno/MMM-MacAddrScan/-/issues
